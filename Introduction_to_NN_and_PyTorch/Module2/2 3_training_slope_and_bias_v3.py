# jupyter nbconvert --to script "2 3_training_slope_and_bias_v3.ipynb"
# #!/usr/bin/env python
# coding: utf-8
...
# Ensure each value in LOSS is detached before converting to numpy
plt.plot([loss.detach().cpu().numpy() if isinstance(loss, torch.Tensor) else loss for loss in LOSS], label="LOSS")

# Similarly for LOSS2
plt.plot([loss.detach().cpu().numpy() if isinstance(loss, torch.Tensor) else loss for loss in LOSS2], label="LOSS2")

#plt.plot(LOSS, label = "LOSS")
#plt.plot([loss.detach().numpy() for loss in LOSS], label="LOSS")
